...
import {
  LayoutDashboard, TicketCheck, CheckCircle2, FileText, CreditCard,
...
const SidebarBlob: React.FC = () => (
  <svg className="sidebar-blob" viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
    <path
      ...
      fill="url(#blobGradClient)"
      opacity="0.5"
    />
    <defs>
      <linearGradient id="blobGradClient" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          {!effectiveCollapsed ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '14px' }}>
              <button
                ...
                style={{
                  background: 'var(--bg-body)', border: '1.5px solid var(--border)', borderRadius: '10px',
                  padding: '7px', cursor: 'pointer', color: 'var(--brand-dark)',
                  alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease',
                }}
              >
          ...
          {!effectiveCollapsed ? (
            <>
              <div style={{ display: 'flex', gap: '7px' }}>
                ...
                >
                  {theme === 'dark'
                    ? <Sun style={{ width: '14px', color: '#D97706' }} />
                ...
                <div ref={notifRef} style={{ position: 'relative', flex: 1 }}>
                  ...
                  {showNotif && (
                    <div style={{
                      ...
                      border: '1px solid var(--border)', padding: '14px', zIndex: 50,
                    }}>
                      ...
                      <div style={{ maxHeight: '240px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {notifications.length === 0
                          ...
                          : notifications.slice(0, 8).map(n => (
                            <div
                              key={n.id}
                              onClick={async () => { await clientApi.markNotificationRead(n.id); fetchNotifs(); }}
                              style={{
              ...
              >
                ...
                <span>{t('logout')}</span>
                <ChevronRight style={{ width: '12px', marginLeft: 'auto' }} />
              </button>
            </>
          ) : (
      ...
      <header className="mobile-top-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button
            onClick={toggleLanguage}
            style={{
              padding: '6px 10px', borderRadius: '10px', background: 'var(--bg-body)',
              border: '1px solid var(--border)', fontSize: '12px', fontWeight: 700,
              color: 'var(--text-body)', cursor: 'pointer',
            }}
          >
            {language === 'ar' ? 'EN' : 'AR'}
          ...
          >
            {theme === 'dark' ? <Sun style={{ width: '14px', color: '#D97706' }} /> : <Moon style={{ width: '14px' }} />}
          </button>
        </div>
      </header>
